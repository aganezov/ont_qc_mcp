...
For coverage grouped by region, rows have a one-to-one ordered correspondence with normalized intervals. Each row's ID, contig, coordinates, and name must match its requested interval. Window-row tiling and correspondence are backend acceptance work for V2-04 rather than an invariant claimed by this contract-only slice.
...
When alignment counts and mapping quality appear together, mapped plus unmapped records and known plus missing MAPQ records each equal the eligible record count. Secondary and supplementary counts are nonexclusive annotations of that population.
