...
def parse_mosdepth_summary(text: str, file_path: str, threshold: float | int | None = None) -> MosdepthStats:
    ...
    # mosdepth can emit per-base bed for thresholds; caller can handle separately
    return MosdepthStats(
        file=file_path,
        mean_depth=sum(c.mean_depth for c in coverage_by_contig) / len(coverage_by_contig) if coverage_by_contig else 0.0,
        median_depth=None,
